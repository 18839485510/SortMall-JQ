...
(function ($) {
    function Carousel($elem, options) {
        ...
        this.loadImgUrl = options.loadImgUrl
        this.activeIndex = 0
        this.now = 0
    ...
    Carousel.prototype = {
        ...
        init: function () {
            ...
            for (var i = 0; i < this.len; i++) {
                ...
                var $img = $('<img>').attr('src', this.loadImgUrl).attr('data-src', this.imgs[i])
            ...
            this.$elem.on('show', function (ev, index) {
                var $imgElem = _this.$carouselImgsChildren.eq(index).find('img')
                var imgSrc = $imgElem.data('src')
